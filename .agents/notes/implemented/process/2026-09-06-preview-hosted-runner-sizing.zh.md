...
已执行的[针对性回归](https://github.com/deepseek-ai/deepseek-harness/blob/master/scripts/preview-workflow.spec.ts) 固定托管路由、PR 触发器与权限、不可变完整构建、只恢复缓存、发布内容整理、受保护镜像检查及幂等评论。实际修改为自托管路由会使路由断言失败；恢复后全部三个测试通过。不改变模型可见运行时行为，因此不需要修改 Session 快照。
