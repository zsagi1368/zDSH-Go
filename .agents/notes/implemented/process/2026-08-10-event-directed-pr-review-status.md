...
[Issue lifecycle](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/issue-lifecycle.yml) remains unsubscribed from `pull_request.ready_for_review`; neither event command depends on that action. [Issue policy](https://github.com/deepseek-ai/deepseek-harness/blob/master/.github/workflows/issue-policy.yml) retains `ready_for_review` because it owns required-check enforcement when a human pull request enters review.
