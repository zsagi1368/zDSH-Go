...
> tester 执行；日期 2026-09-09；worktree `G:\000Github\zDSH\zDSH-wt\zDSH-go`（分支 zDSH-go，起测 HEAD=d87c48f87e，新增 commit 4d3a99d847）。
> 证据文件（worktree 内相对路径）：/tmp 日志已摘录进本报告，关键原始日志见 `del/20260909-053849-t5-config-catalog-gate-fix/` 旁的 ops 交接与下文逐项引用。
