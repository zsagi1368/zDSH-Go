 ...
 *
 * The upstream tree spreads its CI across ci.yml / ci-master.yml / e2e.yml /
 * e2b-e2e.yml / python-release.yml / build-exe-for-python-sdk.yml /
 * issue-lifecycle.yml / issue-policy.yml / release.yml / release-publish.yml /
 * release-vendor.yml / release-vendor-publish.yml / docs-pages.yml. The
 * zDSH-Go fork deliberately removed every upstream workflow (commit 167b35fb1b)
 * and ships exactly ONE workflow: `.github/workflows/ci.yml` (a standalone
 * Windows quality gate). The tests below pin that factual inventory instead of
 * the upstream file set, and keep the upstream checks that still anchor real
 * files in this tree (wine gate script, GitLab CI, lefthook, vitest config).
 */
...
describe('CI workflow (zDSH-Go standalone quality gate)', () => {
  ...
  it('runs the quality gate on main pushes, PRs, and manual dispatch on a Windows runner', () => {
    ...
    // The gate builds the three shipped libraries before running the suites.
    expect(runCommands).toContain('pnpm run build:lib:host')
    expect(runCommands).toContain('pnpm run build:lib:client')
    expect(runCommands).toContain('pnpm run build:lib:plugins')
