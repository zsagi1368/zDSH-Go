...
完整参考（示例与 `~/.zdsh-plugin-center` 数据布局）： [docs/CONFIGURATION.md](docs/CONFIGURATION.md)

## 安全声明

威胁模型、八项保证（目标固定、一次性计划、字节级回滚、脚本门禁、SSRF 守卫、同源 + 意图双闸、无秘密审计、有界重启）以及已接受的残余风险，详见 [SECURITY.md](SECURITY.md)。
...
None, as 插件中心经 Web 控制台驱动浏览器界面与一次性 profile 生命周期事务；不注册任何 prompt、工具或会话事件。

#### KV Cache 影响

无直接影响；安装计划、回滚与审计记录都是宿主侧状态，从不进入模型请求。

## 已知限制与延期工作

- `restart/request` 路由报告 `not_implemented`；现阶段请经看门狗开关路由管理，并手动重启宿主。
- 目标活跃 manifest 拉取失败时，脚本门控回退为目录声明的策略。
- 出站请求只判字面主机；解析地址钉扎是后续工作。
